...
class BitmapAllocatorImpl
   ...
   public:
    ...
    struct BlockChunkHeader {
        ...
        // Must be aligned by sizeof(uint32_t) for use with _BitScanForward
#ifdef _WIN64
        unsigned char bitmap[((BLOCK_CHUNK_COUNT / 8) + 7) & ~7];
        // Lock block
        size_t LockAndGetFreeBlock() {
            for (size_t b = 0; b < _countof(bitmap); b += sizeof(uint64_t)) {
                unsigned long bit = 0;
                const uint64_t& mask(*((uint64_t*)&bitmap[b]));
                if (mask == 0) continue;
                if (!_BitScanForward64(&bit, mask))
                    continue;  // No free bits in this uint32_t
        ...
        // Lock block
        size_t LockAndGetFreeBlock() {
            for (size_t b = 0; b < _countof(bitmap); b += sizeof(uint32_t)) {
                uint32_t bit = 0;
                const uint32_t& mask(*((uint32_t*)&bitmap[b]));
                if (mask == 0) continue;
    ...
    void TestChunkEmptiness(void* chunk) {
        ...
        for (size_t i = 0; i < GetBlockClassCount(); i++) {
            ...
            for (size_t b = 0; b < sizeof(chunkHeader->bitmap); b += sizeof(uint32_t)) {
                if (*((uint32_t*)&chunkHeader->bitmap[b]) != 0xFFFFFFFF)
                    return;
            }
        }
        free(chunk);
